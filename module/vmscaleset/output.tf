...
#   sensitive   = true
#   value       = var.admin_password == null ? element(concat(module.random-password.password.*.result, [""]), 0) : var.admin_password
# }
# output "linux_vm_password" {
#   description = "Password for the Linux VM"
#   sensitive   = true
#   value       = var.disable_password_authentication == false && var.admin_password == null ? element(concat(module.random-password.password.*.result, [""]), 0) : var.admin_password
# }
